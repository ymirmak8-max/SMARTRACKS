...
const PrivacyConsentGate = ({ children }) => {
  ...
  const [notice, setNotice] = useState(cachedNotice);
  ...
  const acceptedVersion = user?.privacyNoticeVersion || null;
  ...
  const loadNotice = useCallback(async () => {
    ...
    setError('');
    if (!cachedNotice) setLoading(true);
    try {
      const response = await api.get('/auth/privacy-notice');
      if (id !== requestId.current) return;
      cachedNotice = response.data.notice;
  ...
  useEffect(() => {
    if (!userId || onAuthScreen) return undefined;
    loadNotice();
    return undefined;
  }, [userId, onAuthScreen, loadNotice]);
  ...
  if (!user || onAuthScreen) return children;

  const noticeVersion = notice?.version;
  const alreadyAccepted = Boolean(acceptedVersion && (!noticeVersion || acceptedVersion === noticeVersion));
  if (alreadyAccepted) return children;

  if (!notice) {
    if (loading || acceptedVersion) {
      ...
      <main className="privacy-page">
        <section className="privacy-card" aria-labelledby="privacy-title">
          ...
          <div className="privacy-load-error" role="alert">
            ...
            <p>{error || 'The privacy notice could not be loaded.'}</p>
  ...
  if (acceptedVersion === notice.version) return children;
    ...
    <main className="privacy-page">
      <section className="privacy-card" aria-labelledby="privacy-title">
        <header className="privacy-header">
          ...
          <small>Notice version {notice.version} · Effective {notice.effectiveDate}</small>
        </header>
